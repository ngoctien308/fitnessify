...
<template>
    <Portal :handleCloseModal="closeModal" v-if="chosenExercise">
        <div class="exercise-description">
            <h3>{{ chosenExercise }}</h3>
            <div>
                <p>{{ chosenExerciseDescription }}</p>
            </div>
            <button @click="closeModal">Close <i class="fa-solid fa-xmark"></i></button>
        </div>
    </Portal>
    <section id="workout-card">
        ...
        <div class="workout-grid">
            ...
            <div class="workout-grid-row" v-for="(w, wIdx) in warmup" :key="wIdx">
                ...
                <p>{{ w.sets }}</p>
                <p>{{ w.reps }}</p>
                <input class="grid-weights" placeholder="14kg" disabled/>
            </div>
